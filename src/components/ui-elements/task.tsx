...
async function finishTask(uid: string, taskId: string, duration: number, startTime: Date | undefined) {
  const ref = doc(db, uid, "userInfo", "tasks", taskId);
  if (startTime) {
    const newDuration = duration + Math.floor((Date.now() - Number(startTime)) / 1000);
...
export default function Task({
  ...
  return (
    <div key={id} className={cn(colorVariants[color], "flex items-center rounded p-2")}>
      ...
      <div className="algin-center flex flex-0 items-center gap-1">
        ...
        {status === "doing" && <LuPause onClick={() => finishTask(uid, id, duration, startTime)} />}
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end">
            <DropdownMenuItem onClick={() => deleteTask(uid, id)}>削除</DropdownMenuItem>
          </DropdownMenuContent>
        </DropdownMenu>
      </div>
    </div>
